...
export default async function ProductsPage({
    ...
    let resolvedParams: { category?: string; search?: string; page?: string } = {};
    ...
    try {
        ...
        // Still try to resolve params for UI
        try {
            resolvedParams = await searchParams;
        } catch {
            // Ignore if params resolution fails
        }
    }
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
                        <div>
                            ...
                            <p className="text-stone-600">Browse our complete collection of premium furniture pieces</p>
                        </div>
                            ...
                            <div className="bg-stone-50 rounded-xl p-6 sticky top-24">
                                ...
                                <div className="space-y-1">
                                    ...
                                    {categories.length > 0 ? categories.map((category) => (
                                        ...
                                        <p className="text-sm text-stone-500 py-2">No categories available</p>
                                    )}
                                </div>
                            </div>
                        </aside>
                                ...
                                <>
                                    ...
                                    {totalPages > 1 && (
                                        <div className="flex justify-center items-center gap-2">
                                            {page > 1 && (
                                                <a
                                                    href={`/products?page=${page - 1}${resolvedParams.category ? `&category=${resolvedParams.category}` : ''}`}
                                            ...
                                            <div className="flex items-center gap-1">
                                                {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                                                    <a
                                                        key={p}
                                                        href={`/products?page=${p}${resolvedParams.category ? `&category=${resolvedParams.category}` : ''}`}
                                ...
                                <div className="text-center py-20 bg-stone-50 rounded-2xl">
                                    ...
                                    {error && (
                                        <p className="text-sm text-stone-400 mt-2">
                                            {process.env.NODE_ENV === 'development' && 'Check your database connection.'}
                                        </p>
                                    )}
                                </div>
                            )}
