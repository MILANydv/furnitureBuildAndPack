      ...
      <div className="bg-white rounded-[10px] p-4 shadow-sm border border-stone-100 flex flex-col lg:flex-row gap-4 items-center">
        <div className="relative flex-1 w-full group">
          ...
          <input
            ...
            className="w-full pl-16 pr-8 py-4 bg-stone-50 border-transparent rounded-[10px] focus:bg-white focus:border-amber-500/20 transition-all outline-none font-bold text-stone-900 text-sm"
          />
        </div>
        <button className="flex items-center gap-2 px-8 py-4 bg-white border border-stone-100 text-stone-400 font-bold rounded-[10px] hover:text-stone-900 transition-all text-xs uppercase tracking-widest">
                ...
                <tr key={product.id} className="hover:bg-stone-50/50 transition-all group">
                  <td className="px-10 py-6">
                    <div className="flex items-center gap-6">
                      <div className="w-16 h-16 rounded-[10px] bg-stone-100 border border-stone-200 overflow-hidden relative group-hover:scale-105 transition-transform">
                        {product.imageUrl ? (
                  ...
                  <td className="px-10 py-6 text-right">
                    <div className="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0">
                      <Link href={`/shop/products/${product.slug}`} target="_blank" className="p-3 text-stone-400 hover:text-stone-900 hover:bg-white rounded-[10px] shadow-sm transition-all border border-transparent hover:border-stone-100">
                        <Eye className="w-4.5 h-4.5" />
                      </Link>
                      <Link href={`/admin/products/${product.id}/edit`} className="p-3 text-stone-400 hover:text-blue-600 hover:bg-white rounded-[10px] shadow-sm transition-all border border-transparent hover:border-stone-100">
                        <Edit className="w-4.5 h-4.5" />
                      </Link>
                      <button
                        onClick={() => { if (confirm('Purge this product?')) deleteMutation.mutate(product.id) }}
                        className="p-3 text-stone-400 hover:text-red-600 hover:bg-white rounded-[10px] shadow-sm transition-all border border-transparent hover:border-stone-100"
                      >
