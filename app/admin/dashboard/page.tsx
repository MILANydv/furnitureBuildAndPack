...
export default function AdminDashboard() {
  ...
  const dashboardStats = [
    { label: 'Total Revenue', value: formatPrice(stats.totalRevenue), icon: DollarSign, trend: '+0%' },
    { label: 'Total Orders', value: stats.totalOrders.toString(), icon: ShoppingCart, trend: '+0%' },
    { label: 'Total Products', value: stats.totalProducts.toString(), icon: Box, trend: '+0%' },
    { label: 'Active Customers', value: stats.totalCustomers.toString(), icon: Users, trend: '+0%' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <nav className="bg-white rounded-xl shadow-sm p-4 space-y-1">
              {navItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${item.active ? 'bg-amber-50 text-amber-700' : 'text-stone-600 hover:bg-stone-50'
                    }`}
                >
          ...
          <main className="flex-1">
            <h1 className="text-2xl font-bold text-stone-900 mb-6">Dashboard Summary</h1>
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-stone-200">
                    ...
                    {stats.recentOrders.length === 0 && (
                      <tr>
                        <td colSpan={5} className="px-6 py-4 text-center text-stone-500 italic">No orders yet</td>
                      </tr>
                    )}
                  </tbody>
                </table>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <Link href="/admin/products/new" className="bg-white rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex items-center gap-4">
              ...
              <Link href="/admin/coupons/new" className="bg-white rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex items-center gap-4">
