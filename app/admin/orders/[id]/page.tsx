...
export default function OrderDetailPage({ params }: { params: { id: string } }) {
    ...
    if (isLoading) return <div className="animate-pulse space-y-8">
        <div className="h-20 bg-stone-100 rounded-3xl w-full"></div>
        <div className="grid grid-cols-3 gap-8">
            <div className="h-96 bg-stone-100 rounded-3xl col-span-2"></div>
        ...
        <div className="space-y-10 pb-24 max-w-6xl mx-auto">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                <div className="flex items-center gap-6">
                    <button onClick={() => router.back()} className="p-4 bg-white border border-stone-100 rounded-2xl text-stone-900 hover:bg-stone-50 transition-all shadow-sm">
                ...
                <div className="flex gap-4">
                    <button className="px-6 py-3 bg-white border border-stone-200 rounded-2xl text-stone-900 font-bold hover:bg-stone-50 transition-all text-xs uppercase tracking-widest flex items-center gap-2 shadow-sm">
            ...
            {/* Status Timeline */}
            <div className="bg-white p-10 rounded-[2.5rem] border border-stone-100 shadow-sm overflow-hidden">
                    ...
                    {statusFlow.slice(0, 4).map((step, i) => {
                        ...
                        return (
                            <div key={step.value} className="flex flex-col items-center gap-4">
                                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-500 ring-8 ring-white ${isPast || isCurrent ? 'bg-amber-500 text-white shadow-xl shadow-amber-500/30' : 'bg-stone-100 text-stone-300'}`}>
            ...
            <div className="grid lg:grid-cols-3 gap-10">
                        ...
                        <div className="divide-y divide-stone-50">
                            {order.items.map((item: any) => (
                                <div key={item.id} className="p-8 flex items-center justify-between group">
                                    <div className="flex items-center gap-6">
                                        <div className="w-20 h-20 bg-stone-50 rounded-[1.5rem] border border-stone-100 overflow-hidden">
                        ...
                        <div className="grid sm:grid-cols-2 gap-4">
                            {statusFlow.map((s) => (
                                <button
                                    ...
                                    className={`p-6 rounded-2xl flex items-center justify-between border transition-all ${order.status === s.value ? 'bg-amber-50 border-amber-500/20 text-amber-900' : 'bg-stone-50 border-transparent hover:border-stone-200 text-stone-600'}`}
                                >
                ...
                <div className="space-y-10">
                    <section className="bg-[#1A1C1E] p-10 rounded-[2.5rem] text-white shadow-2xl space-y-8">
                        <div className="flex items-center gap-4">
                        ...
                        <div className="space-y-6">
                            <div className="flex items-center gap-4">
                                <div className="w-10 h-10 bg-white/5 rounded-xl flex items-center justify-center text-stone-400">
                    ...
                    <section className="bg-white p-10 rounded-[2.5rem] border border-stone-100 shadow-sm space-y-8">
                        ...
                        <button className="w-full py-4 bg-stone-900 text-white font-black rounded-2xl hover:bg-stone-800 transition-all text-[10px] uppercase tracking-[0.2em] shadow-xl shadow-stone-900/10">
